...
//     If no offered skill is preferred but at least one is neutral (not blacklisted and not preferred), choose the neutral
...
function pick(preferred, blacklisted, options) {
    ...
    const mapped = options.map(([skill, percentage], index) => ({
        skill,
        percentage,
    ...
    const allowed = mapped.filter(
        option => !blacklisted.has(option.skill)
    );
    ...
    const preferredOptions = allowed.filter(
        option => preferred.has(option.skill)
    );

    if (preferredOptions.length > 0) {
        return preferredOptions.reduce((best, cur) =>
